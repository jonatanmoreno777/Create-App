# app.R - ARCHIVO COMPLETO Y FUNCIONAL
# Configurar repositorios para weathercan
# Configurar repositorios primero
options(
  repos = c(
...
server <- function(input, output, session) {
  ...
  station_meta <- reactiveFileReader(intervalMillis = 1000, 
                                     session,
                                     filePath = "D:/Python_R/R/App/Data/stations.rds", 
                                     readFunc = readRDS
  )
  ...
  current_rds_date <- reactivePoll(1000, session,
                                   ...
                                   checkFunc = function() {
                                     file.info("D:/Python_R/R/App/Data/stations.rds")$mtime[1] 
                                   },
      ...
      shinyalert(
        ...
        callbackR = function(value) {
          if(value == TRUE) {
            ...
            file.copy(meta_sys_rds, "D:/Python_R/R/App/Data")
...
stations_data <- readRDS("D:/Python_R/R/App/Data/stations.rds")

# Ver la estructura del objeto
str(stations_data)
# Ver las clases de cada elemento
class(stations_data)
names(stations_data)

# El elemento 'stn' YA ES un DataFrame (tibble)
stations_df <- stations_data$stn

# Ver como DataFrame normal
View(stations_df)  # Abre en una pestaña de visualización

# O ver en consola
print(stations_df)

# Dimensiones
cat("Filas:", nrow(stations_df), "Columnas:", ncol(stations_df), "\n")
