...
class ANDREA:
    ...
    def _get_station_details(self, station_id):
        """Obtiene detalles completos de una estación - CORREGIDO"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'd' in result:
                    ...
                    # CORREGIDO: Seguir la estructura real del API
                    if detalles and 'Lista' in detalles and len(detalles['Lista']) > 0:
                        estacion_data = detalles['Lista'][0]
                        # Mapear campos según el código interno de ANDREA
                        return {
                            'OPERADOR': estacion_data.get('OPERADOR', ''),
                            'COOLONGITUD': estacion_data.get('COOLONGITUD', 0),
                            'COOLATITUD': estacion_data.get('COOLATITUD', 0),
                            'ALTITUD': estacion_data.get('ALTITUD', 0),
                            'NOMBRE': estacion_data.get('NOMBRE', ''),
                            'IDESTACION': estacion_data.get('IDESTACION', '')
                        }
        except Exception as e:
    ...
    def _get_station_operator_from_variables(self, station_id):
        """Obtiene el operador desde las variables de la estación - NUEVO MÉTODO"""
        variables = self._get_station_variables(station_id)
        if variables:
            # Tomar el operador de la primera variable
            for variable in variables:
                operador = variable.get('OPERADOR', '')
                if operador:
                    return operador
        return ''
    ...
    def _get_stations_by_basin(self, basin_name, data_type, source):
        ...
        for estacion in estaciones:
            ...
            # Obtener operador desde las variables - NUEVO ENFOQUE
            operador = self._get_station_operator_from_variables(station_id)
            ...
            if detalles:
                ...
                if tipo_estacion:  # Solo incluir si coincide con el tipo solicitado
                    estacion_info = {
                        ...
                        'operator': operador,  # Usar operador desde variables
    ...
    def _get_station_variables(self, station_code):
        ...
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                if 'd' in result:
                    variables_data = json.loads(result['d'])
...
estaciones_biabo = list_stations(basin_name='Mantaro')
estaciones_biabo
...
class Plot:
    ...
    @staticmethod
    def spatial_stations(list_stations, mapbox_access_token=None):
        """
        ...
        mapbox_access_token : str, optional
            Mapbox access token for better basemaps
        ...
        fig = go.Figure(go.Scattermapbox(
            lat=list_stations.Latitude.to_list(), 
            lon=list_stations.Longitude.to_list(),
            mode='markers', 
        ...
        if mapbox_access_token:
            ...
            center_lat = list_stations.Latitude.mean()
            center_lon = list_stations.Longitude.mean()
            ...
            fig.update_layout(
                geo=dict(
                    ...
                    lonaxis=dict(showgrid=True, gridwidth=0.5, range=[list_stations.Longitude.min()-1, list_stations.Longitude.max()+1]),
                    lataxis=dict(showgrid=True, gridwidth=0.5, range=[list_stations.Latitude.min()-1, list_stations.Latitude.max()+1]),
                )
            )
    ...
    # ========== MÉTODOS PLOTLY (Nuevos) ==========

    @staticmethod
    def time_series_plotly(data, station_name=None, data_type='flow', title=None):
        """
        ...
        # Set y-axis label
        ylabel = 'River Discharge [m³/s]' if data_type == 'flow' else 'Precipitation [mm]'
        ...
        if title is None:
            title = f'{station_display} - {data_type.capitalize()} Time Series'
    ...
    @staticmethod
    def monthly_boxplot_plotly(data, data_type='flow', title=None):
        """
        Monthly boxplot using Plotly.
        ...
        for month in month_names:
            month_data = df_plot[df_plot['Month'] == month]['Value']
            if len(month_data) > 0:
        ...
        if title is None:
            title = f'Monthly {data_type.capitalize()} Distribution'
    ...
    @staticmethod
    def comparison_plotly(data_dict, data_type='flow', title=None):
        """
        Compare multiple time series using Plotly.
        ...
        if title is None:
            title = f'{data_type.capitalize()} Comparison'
